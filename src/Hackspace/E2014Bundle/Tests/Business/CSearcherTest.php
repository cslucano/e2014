...
class CSearcherTest extends WebTestCase
{
    ...
    public function testPopulateEsFacetResults()
    {
        ...
        $cSearcher = $container->get('hackspace_e2014.c_searcher');

        $this->setPropertyValue($cSearcher, 'facets', $this->facets);

        $cSearcher->populateEsFacetResults($this->facetResult);

        $facets = $this->getPropertyValue($cSearcher, 'facets');
